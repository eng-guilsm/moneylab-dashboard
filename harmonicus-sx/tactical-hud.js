...
function renderLiveThermometer(plans, filter) {
  ...
  container.innerHTML = sortedPlans.map((plan, idx) => {
    ...
    const badgeText = isDesativado ? '⏸️ DESATIVADO PELA GOVERNANÇA' : (plan.badge ? plan.badge : (isLow ? '🛡️ BAIXO RISCO' : '⚡ MÉDIO RISCO'));
    ...
    return `
      <div class="thermo-card${cardExtraClass}" data-plan-id="${plan.id}" style="border-top: 3px solid ${borderTopColor}; ${cardExtraStyle}" title="${isDesativado ? 'Plano temporariamente desativado pela Governança' : 'Clique para abrir análise executiva e histórico'}">
        ...
        <div>
          <div class="thermo-name" style="${isDesativado ? 'color: #CBD5E1;' : ''}">${plan.nome.startsWith(plan.icone) ? plan.nome : `${plan.icone} ${plan.nome}`}</div>
  ...
  content.innerHTML = `
    <div class="modal-header-box">
      <div class="modal-title-left">
        <span class="plan-badge ${badgeClass}">${badgeText}</span>
        <h3 class="modal-plan-title">${plan.nome.startsWith(plan.icone) ? plan.nome : `${plan.icone} ${plan.nome}`}</h3>
